...
class SetCubeBuilder:

    def __init__(self, setCode, filename, mythics, rares, uncommons, commons):
    ...
    def openFile(self, filename):
        ...
        self.f = open('cardnames.txt', 'w+')

    def callQueries(self):

        #call queries for each rarity
        self.queries(self.setCode, 'Mythic', f)
        self.queries(self.setCode, 'Rare', f)
        self.queries(self.setCode, 'Uncommon', self.f)
        self.queries(self.setCode, 'Common', self.f)
    ...
    def getNames(self, fetch, number, f):
        ...
        for card in cardList:
            ...
            if 'promo_types' not in card:
                ...
                self.totalCards = self.totalCards + number
                f.write(str(number))
                f.write(' ')
                f.write(card['name'])
                f.write('\n')
    ...
    def queries(self, setCode, full_rarity, f):
